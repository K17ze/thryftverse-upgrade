...
import { fetchSellerAnalytics, fetchTopPerformers, fetchNeedsAttention, type SellerAnalytics, type TopPerformerListing, type NeedsAttentionListing } from '../services/commerceApi';
...
export default function SellerAnalyticsScreen() {
  ...
  // ── Listing context (P2-06) ──
  // When navigated from Manage Listing or Seller Hub, the source listing is
  // preserved so the header can scope to that item instead of dropping context.
  const { listingId, listingTitle, listingImage } = route.params ?? {};
  const scopedListing = useMemo(
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [listingsRes, analyticsData, topData, attentionData] = await Promise.all([
        ...
        fetchTopPerformers(currentUser.id, 10, period).catch(() => null),
        fetchNeedsAttention(currentUser.id, 5, period).catch(() => null),
      ]);
  ...
  const kpiRows = useMemo(() => {
    const rows: { label: string; value: string }[] = [
      { label: 'Items sold', value: itemsSold != null ? String(itemsSold) : '—' },
      {
        ...
        value: avgOrderValue != null ? formatFromFiat(avgOrderValue, 'GBP') : '—',
      },
      {
        label: 'Conversion',
    ...
    return rows;
  }, [itemsSold, avgOrderValue, conversionRate, totalViews, activeListings, avgRating, reviewCount, formatFromFiat]);
  ...
  // ── Needs attention: period-scoped from backend API ──
  // Enriched with imageUrl from listings data for media-first rows.
  ...
  const handleListingPress = useCallback((listingId: string) => {
    openProductDetail(navigation, { referenceKind: 'listing', canonicalId: listingId, sourceSurface: 'SellerAnalytics' });
  }, [navigation]);
  ...
  if (isLoading) {
    return (
      <FlagshipScreen
        header={<FlagshipHeader title={scopedListing ? 'Listing Analytics' : 'Seller Analytics'} onBack={() => navigation.goBack()} />}
      >
        <ScrollView
  ...
  return (
    ...
    >
      ...
      >
        {/* ── Listing context banner (P2-06) ──
            When navigated from a specific listing, show its identity so the
            seller never loses the source context. Flat row, no card chrome. */}
        {scopedListing ? (
          <View style={[styles.listingContextRow, { borderBottomColor: colors.border }]}>
            {scopedListing.image ? (
              <CachedImage
                uri={scopedListing.image}
                style={styles.listingContextThumb}
                contentFit="cover"
              />
            ) : (
              <View style={[styles.listingContextThumb, { backgroundColor: colors.surfaceAlt }]} />
            )}
            <Text style={[styles.listingContextTitle, { color: colors.textPrimary }]} numberOfLines={2}>
              {scopedListing.title ?? 'Listing'}
            </Text>
          </View>
        ) : null}
        ...
        <View style={styles.sectionHeader}>
          ...
          <Text style={[styles.sectionHint, { color: colors.textMuted }]}>Last {periodLabel}</Text>
        </View>
        {topPerformersError ? (
        ...
        {/* ── Needs attention — period-scoped, flat rows ── */}
        {needsAttentionError ? (
          <View>
            <View style={styles.sectionHeader}>
            ...
            <View style={styles.sectionErrorWrap}>
              <Text style={[styles.sectionErrorText, { color: colors.textMuted }]}>
                Couldn't load attention data. Pull to retry.
              </Text>
            </View>
          </View>
