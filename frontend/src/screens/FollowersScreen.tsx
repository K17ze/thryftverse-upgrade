...
import { useFollowersInfinite, useFollowMutation } from '../platform/server';
...
export default function FollowersScreen() {
  ...
  const followersQuery = useFollowersInfinite(userId);

  const items: FollowListUser[] = useMemo(() => {
    const pages = followersQuery.data?.pages ?? [];
  ...
  const isLoading = followersQuery.isLoading && items.length === 0;
  const hasError = Boolean(followersQuery.error) && items.length === 0;
  const hasNextPage = Boolean(followersQuery.hasNextPage);
  const isFetchingNextPage = followersQuery.isFetchingNextPage;

  const handleLoadMore = useCallback(() => {
    if (hasNextPage && !isFetchingNextPage) followersQuery.fetchNextPage();
  }, [hasNextPage, isFetchingNextPage, followersQuery]);
  ...
  return (
    ...
    >
      ...
      <FlatList
        ...
        ListEmptyComponent={
          isLoading ? (
            <View accessibilityLabel="Loading followers">
            ...
            <EmptyState
              ...
              ctaLabel="Retry"
              onCtaPress={() => void followersQuery.refetch()}
            />
          ) : searchQuery.length > 0 ? (
            <EmptyState
              density="compact"
              icon="search-outline"
              title="No matches"
              subtitle={`No followers match "${searchQuery}".`}
              iconColor={colors.textMuted}
            />
          ) : (
            <EmptyState
...
function FollowButton({
  ...
  const isFollowing = followMutation.variables ?? false;
