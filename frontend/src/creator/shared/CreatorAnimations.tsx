...
export function SheetContainer({
  ...
  // The off-screen target is the live window height — never a hardcoded
  // constant — so entrance/exit fully clear the viewport on any device
  // (a hardcoded 1000 leaves a >1000px sheet peeking, and overruns on
  // short windows).
  const translateY = useSharedValue(windowHeight);
  const backdropOpacity = useSharedValue(0);
  const mountedRef = useRef(false);
  // Shared values (not JS refs) — these are read/mutated inside UI
  // worklets, where plain useRef access is not thread-safe.
  const sheetHeightSV = useSharedValue(windowHeight * effectiveMaxHeight);
  ...
  useEffect(() => {
    sheetHeightSV.value = windowHeight * effectiveMaxHeight;
  }, [windowHeight, effectiveMaxHeight, sheetHeightSV]);
  ...
  useEffect(() => {
    if (visible) {
      ...
      if (reduceMotion) {
        ...
        translateY.value = withTiming(windowHeight, { duration: Motion.duration.slow, easing: Easing.in(Easing.ease) });
        backdropOpacity.value = withTiming(0, { duration: Motion.duration.normal });
      }
    }
  }, [visible, reduceMotion, translateY, backdropOpacity, windowHeight, isDismissingSV]);
  ...
  const panGesture = Gesture.Pan()
    ...
    .onEnd((e) => {
      ...
      // The threshold reads the measured sheet height live on the UI
      // thread — a render-time capture could be stale by a frame.
      const dismissThreshold = Math.max(100, sheetHeightSV.value * 0.25);
      const shouldDismiss = e.translationY > dismissThreshold || e.velocityY > DISMISS_VELOCITY;
      if (shouldDismiss) {
