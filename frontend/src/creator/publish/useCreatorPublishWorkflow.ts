...
import { uploadAllLocalMedia, hasLocalUris } from '../core/upload/mediaUploadPipeline';
...
function scanDocumentForLocalUris(doc: CreatorDocument): LocalMediaRef[] {
  ...
  // Canvas background image is document-scoped (not a layer) — it still
  // carries a device URI that must be uploaded before publish.
  const bg = doc.canvas?.background;
  if (bg?.type === 'image' && typeof bg.value === 'string' && isLocalUri(bg.value)) {
    refs.push({ layerId: CANVAS_LAYER_ID, field: 'backgroundValue', currentUri: bg.value, assetType: 'image' });
  }
  for (const page of doc.pages) {
    for (const layer of page.layers) {
      // Alpha-mask PNG (cutout) — a local mask URI must upload before
      // publish or the server render produces an uncut layer.
      if (typeof layer.maskRef === 'string' && isLocalUri(layer.maskRef)) {
        refs.push({ layerId: layer.id, field: 'maskRef', currentUri: layer.maskRef, assetType: 'image' });
      }
      if (layer.type === 'media') {
