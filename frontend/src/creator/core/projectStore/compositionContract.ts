...
import type { CreatorStoryCreateFrame } from '../../publish/publishTypes';
...
export function validateDocumentStructure(doc: CreatorDocument): ContractValidationResult {
  ...
  // 9. Product tags must bind a real listing. An empty listingId
  // publishes a dead 'listing' sticker — viewers can tap it but it
  // resolves to nothing (shipped templates carry placeholder product
  // layers that must be bound or removed before publish).
  for (const page of validated.pages) {
    for (const layer of page.layers) {
      if (layer.type === 'product' && !layer.payload.listingId) {
        errors.push(
          `Layer ${layer.id}: product tag has no listing — select a listing or remove the tag before publishing`,
        );
      }
    }
  }
...
export function validateForPublish(doc: CreatorDocument): ContractValidationResult {
  ...
  // Canvas background image — document-scoped media that must be remote.
  const bg = validated.canvas?.background;
  if (bg?.type === 'image' && typeof bg.value === 'string' && isLocalUri(bg.value)) {
    errors.push('Canvas background image is a local URI — must be uploaded before publish');
  }
...
export function serialiseToLookPayload(doc: CreatorDocument): {
  ...
  // Hidden layers are not rendered by the editor or viewer — publishing
  // them as frame media would ship content the canvas explicitly hides.
  const mediaLayers = page.layers.filter((l) => l.type === 'media' && !l.hidden);
  ...
  const frames: CreatorStoryCreateFrame[] = doc.pages.map((page, i) => {
    ...
    return {
      ...
      stickers: page.layers
        .filter((l) => l.type !== 'media' && !l.hidden && !isCaptionLayer(l))
        .map((l, si) => {
