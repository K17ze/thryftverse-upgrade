...
import { safeValidateDocument, POSTER_DEFAULT_ASPECT_RATIO, LOOK_DEFAULT_ASPECT_RATIO, LOOK_DEFAULT_BACKGROUND, POSTER_DEFAULT_BACKGROUND } from '../core/projectStore/composition';
...
export interface PosterFrameViewData {
  ...
  mediaUrl?: string;
  /** Still-image rendition for video frames (JPEG poster). Preferred over thumbnailUrl. */
  posterUrl?: string;
  thumbnailUrl?: string;
  caption?: string;
    ...
    if (frame.mediaUrl) {
      layers.push({
        ...
        payload: {
          ...
          contentFit: 'cover',
          opacity: 1,
          // Preserve video metadata when viewing legacy posters — the HLS
          // poster still is the correct thumbnail for m3u8 playback URLs.
          ...((frame.posterUrl ?? frame.thumbnailUrl) ? { thumbnailUri: frame.posterUrl ?? frame.thumbnailUrl } : {}),
