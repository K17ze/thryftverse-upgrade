...
export function pageWithRenderedMedia(
  ...
  for (const layer of page.layers) {
    ...
    if (!layer.hidden && LIVE_VIEW_LAYER_TYPES.has(layer.type)) {
      layers.push(layer);
    }
    // Baked static overlays (text, draw, gif, decorative, countdown,
    // location, hashtag, adjustment) are already in the artifact. Hidden
    // live layers are also dropped — the author hid them; viewers must
    // not see a tappable sticker over the baked frame.
  }
