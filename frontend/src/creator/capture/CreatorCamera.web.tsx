...
export interface CreatorCameraProps {
  ...
  /** Accepted for prop parity with the native camera — the web surface
   *  delegates acquisition to the system picker, which has its own
   *  multi-select, so these are intentionally unused. */
...
export default function CreatorCameraWeb({
  ...
  const noun = mode === 'look' ? 'look'
    : mode === 'poster' ? 'poster'
    : mode === 'moodboard' ? 'moodboard'
    : 'search';
