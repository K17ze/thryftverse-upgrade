...
export function CreatorCanvas({
  ...
  const layerPosSVs = useRef<Record<string, { x: SharedValue<number>; y: SharedValue<number> }>>({});
  ...
  // Clip-relative playback time — layer timeRanges and keyframes are
  // authored in page-clip-relative ms, while currentTimeMs is the global
  // timeline clock. Translate once so the evaluator sees authored units;
  // without this, timed overlays/adjustments on page 2+ never activate
  // during playback. activeClip is null for single-page/legacy callers,
  // where currentTimeMs is already page-relative.
  ...
  const resolvedScene: ResolvedScene = useMemo(
    () =>
      evaluateScene({
        ...
        compareOriginal }),
    [document, page, clipTimeMs, canvasWidth, canvasHeight, renderProfile, compareOriginal],
  );
...
interface LayerRendererProps {
   ...
   *  delta release. */
  layerPosSVs?: React.MutableRefObject<Record<string, { x: SharedValue<number>; y: SharedValue<number> }>>;
...
function commitPeersOnUI(
  registry: Record<string, { x: SharedValue<number>; y: SharedValue<number> }>,
...
const LayerRenderer = React.memo(function LayerRenderer({
  ...
  const panGesture = useMemo(
    ...
    [mode, layer.locked, layer.id, translateX, translateY, startX, startY, onPress, handlePositionCommit, isMultiSelectActive, onMultiDragStart, onMultiDragCommit, commitMultiDrag, canvasWidth, canvasHeight, manipulationActiveSV, onManipulationChange, isInTrashZoneSV, onTrashZoneEnter, onDelete, haptic, reducedMotion, liftSV, snapTargetX, snapTargetY, didSnap, multiDrag, commitDispatchedSV]
  );
...
function MediaLayerContent({
  ...
  const clipOutDurationMs = activeClip?.durationMs
    ?? (payload.trimEndMs != null && payload.trimStartMs != null
      ...
      : 0);
  const fadeGain = (() => {
    if (!hasPlaybackClock || (fadeInMs <= 0 && fadeOutMs <= 0) || clipOutDurationMs <= 0) return 1;
    const t = clipRelMs;
    let g = 1;
    if (fadeInMs > 0) g = Math.min(g, Math.max(0, t / fadeInMs));
    if (fadeOutMs > 0) g = Math.min(g, Math.max(0, (clipOutDurationMs - t) / fadeOutMs));
    return g;
  })();
  const effectiveVolume = Math.max(0, Math.min(1, volume * fadeGain));
