...
| `thryftverse_search_index_lag_seconds` | histogram | `backend`, `outcome` (`submitted`/`completed`/`fallback`) | `src/lib/searchSync.ts` (`syncSingleListing` submission path and detached `confirmIndexWrite` task-confirmation path) | Lag between a listing's `updated_at` and the labelled stage. **`submitted` is submission lag** — `addDocuments` resolved (Meilisearch task enqueued), the document is NOT yet searchable. **`completed` is visibility lag** — the task was confirmed `succeeded` via `/tasks/{taskUid}`. **`fallback`** is a process-local index write after remote failure | `outcome="completed"` p95 > 60s/10m → `SearchIndexStale` (grafana-alerts.yml #9). Do NOT alert on the unlabelled/`submitted` series as freshness — that was audit §4.7's mislabeled-submission defect |
| `thryftverse_search_sync_total` | counter | `op` (`index`/`remove`), `result` (`ok`/`error`), `outcome` (`submitted`/`completed`/`failed`/`fallback`) | `src/lib/searchSync.ts` (`syncSingleListing`, `confirmIndexWrite`) | Per-listing write outcome honestly classified: `submitted` = remote task enqueued; `completed` = task confirmed applied; `failed` = write threw or task failed/canceled; `fallback` = process-local index write after remote failure. `result` is retained for existing dashboards (`ok` ↔ submitted/completed/fallback, `error` ↔ failed) | `result="error"` rate > 5%/10m → `SearchSyncFailing` (#10); submitted≫completed/15m → `SearchIndexWriteStalled` (#9b); `fallback` rate > 0/10m → `SearchSyncFallbackActive` (#9c) |
|| `thryftverse_search_reindex_lease_total` | counter | `outcome` (`acquired`/`contended`/`acquire_error`/`heartbeat_lost`/`released`/`release_missed`/`release_error`) | `src/lib/searchSync.ts` (`reindexListingsBlueGreen` acquire/heartbeat/release paths) | Durable fenced reindex lease (migration 338 `search_reindex_lease`) lifecycle — correct through PgBouncer transaction pooling, replacing the session advisory lock retired under audit S2. `contended` = a live lease exists (normal under overlap); `heartbeat_lost`/`release_missed` = the row changed hands under this process | `heartbeat_lost`/`release_missed` > 0 → `SearchReindexLeaseLost` (#11) |
