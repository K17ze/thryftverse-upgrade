...
### Native architecture context (AGENTS.md §39.4–39.6)

When classifying native fingerprint changes, account for this repository's architecture:

- **Local Expo Module** (`modules/thryft-native/`): custom Swift/Kotlin native code
  via Expo Modules API. Autolinked — no manual registration. A change to this module's
  native code is a binary change, not OTA.
- **`react-native-shared-hero`** (Fabric-native shared element transitions): 18 files
  still use Reanimated's `sharedTransitionTag`. Until all 18 migrate to
  `SharedHeroWrapper`, `ENABLE_SHARED_ELEMENT_TRANSITIONS` remains `true`. A change to
  shared element transition config is a native dependency change, not a safe OTA.
- **iOS prebuild** cannot run on Windows (requires macOS). iOS native projects are
  generated by EAS Build cloud workers. Local iOS testing requires a macOS machine or
  EAS Build.
