...
- **Do not build monolithic kitchen-sink screens.** If a screen exceeds 400 lines or attempts to combine more than 3 distinct sub-domains (e.g. Identity + Live Tasks + Financial Liquidity + Inventory + Analytics + Settings), it must be factored into domain-isolated components in `src/components/<feature>/`. The screen file acts solely as an orchestrator for state, telemetry, and layout composition.
- **Enforce the Surface Budget strictly.** In utility and merchant command surfaces, use flat canvas, hairline separators, and typography scale as the default visual hierarchy. Wrapping every section in rounded grey rectangles creates card-fatigue and signals amateur bot-generation.
- **Unified Action Centers.** Consolidate directly coupled actions (e.g., dispatch deadlines, SLA standards, and order to-dos) into a single unified operational radar rather than spreading them across 3 separate boxes.
