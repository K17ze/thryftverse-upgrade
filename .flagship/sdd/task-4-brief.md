# Task 4 Brief — Human-touch micro-polish: real upload progress on profile media, coaching-copy removal

## Context

Repo root: `C:\Users\User\Desktop\thryftverse-upgrade` (React Native / Expo / TypeScript). This is the final polish wave of an editor/upload flagship campaign. The transport layer reports REAL byte progress (`useProfileMediaUpload` computes `progress` 0–1 from XHR bytes) but profile surfaces collapse it to a binary spinner — which reads as fake. Coaching copy ("Keep the app open", "Review the edges…") is a rejected pattern.

## Requirements

### 1. FlagshipProfileMedia.tsx (`frontend/src/components/flagship/FlagshipProfileMedia.tsx`)
- Find where `isUploadingCover` (or equivalent) renders `<ActivityIndicator>`. Replace with a REAL determinate ring driven by the byte progress the host already receives (check the component's props — if it only receives a boolean, extend the prop to accept `uploadProgress?: number` and update ALL call sites to pass the real value; find call sites via grep for the component name).
- Ring: same grammar as the listing ring — 28pt, 2.5pt stroke, white arc on `rgba(0,0,0,0.35)` circle, `react-native-svg` (verified installed) + Reanimated.
- On complete: fade out, no badge.

### 2. EditProfilePreview.tsx (`frontend/src/components/profile/EditProfilePreview.tsx`)
- Same replacement: binary ActivityIndicator → real progress ring (or the same minimal determinate treatment) wherever `isUploading`-style booleans hide a real progress value. If the hook's progress value isn't threaded into this component, thread it minimally through props — do not rewire the upload stack.

### 3. Coaching-copy removal in `frontend/src/screens/AIPhotoEnhancementScreen.tsx` (COPY ONLY — do not restructure this screen)
- Remove the "Keep the app open" line (lines ~515–523 area).
- Remove "Review the edges around the product to ensure nothing was cut off." (line ~722 area).
- Remove "Drag the slider to compare Â· {label}" instructional prefix — keep the operation label only if it's meaningful provenance, else remove the line entirely.
- Keep the honest provenance/disclosure line (provider + disclosure type) — that is trust information, not coaching. If provenance renders as two stacked labels, collapse to ONE line.
...
Full report → `.flagship/sdd/task-4-report.md`. Return: status, files changed, one-line test summary, concerns.
